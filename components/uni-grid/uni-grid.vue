<template>
	<view :class="{'uni-grid-no-border':!showBorder,'uni-grid-no-out-border':showBorder && !showOutBorder}" class="uni-grid">
		<view v-for="(items,i) in gridGroup" :key="i" class="uni-grid__flex">
			<view v-for="(item,index) in items" :hover-start-time="20" :hover-stay-time="70" :key="index" :class="[index == columnNum ? 'uni-grid-item-last' : '','uni-grid-item-' + type]" :style="{visibility:item.visibility ? item.visibility : 'inherit',background: item.backgroundColor ? item.backgroundColor : ''}" class="uni-grid-item" hover-class="uni-grid-item-hover" @click="onClick(i,index)">
